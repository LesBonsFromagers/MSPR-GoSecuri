    ...
    stages {
        ...
        stage('Compilation') 
        {
            steps 
            {
                ...
                //sh "mvn clean"
                sh "pwd"
                sh "sshpass -v -p \"root\" ssh -v -o \"StrictHostKeyChecking=no\" root@192.168.1.170 'pwd; java /var/lib/jenkins/workspace/MSPRFolder/Pipeline-MSPR/src/main/java/com/epsi/gosecuri/Main.java' "
