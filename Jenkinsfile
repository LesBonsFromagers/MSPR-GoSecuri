    ...
    stages {
        ...
        stage('Copy') 
        {
            steps 
            {
                ...
                //sh "mvn clean"
                sh "pwd"
                sh "sshpass -v -p \"root\" scp -v -o \"StrictHostKeyChecking=no\" /var/lib/jenkins/workspace/MSPRFolder/Pipeline-MSPR/target/untitled104-1.0-SNAPSHOT.jar root@192.168.1.170:/mnt/www/html"
        ...
        stage('Launch') 
        {
            steps 
            {
                ...
                // Run Maven on a Unix agent.
                sh "sshpass -v -p \"root\" ssh -v -o \"StrictHostKeyChecking=no\" root@192.168.1.170 'cd /mnt/www/html/; pwd; javac -jar untitled104-1.0-SNAPSHOT.jar'"
